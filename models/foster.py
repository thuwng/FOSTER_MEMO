...
class FOSTER(BaseLearner):
    ...
    def _feature_compression(self, train_loader, test_loader):
        ...
        optimizer = optim.SGD(filter(
            lambda p: p.requires_grad, self._snet.parameters()), lr=self.args["lr"], momentum=0.9, weight_decay=0)
        scheduler = optim.lr_scheduler.CosineAnnealingLR(
        ...
        print("darknet eval: ")
        print('CNN top1 curve: {}'.format(cnn_accy['top1']))
        print('CNN top5 curve: {}'.format(cnn_accy['top5']))
